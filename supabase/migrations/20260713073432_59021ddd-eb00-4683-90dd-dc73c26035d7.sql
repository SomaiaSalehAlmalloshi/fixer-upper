...
-- Risk weight rules (the engine's lookup table)
CREATE TABLE public.risk_weight_rules (
  ...
  rating TEXT,
  risk_weight NUMERIC(6,4) NOT NULL, -- e.g. 0.20 = 20%
  description TEXT,
...
CREATE POLICY "assets creator or admin update" ON public.rwa_assets FOR UPDATE TO authenticated
  USING (created_by = auth.uid() OR public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'approver'))
  WITH CHECK (true);
CREATE POLICY "assets admin delete" ON public.rwa_assets FOR DELETE TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Approvals log (workflow)
CREATE TABLE public.rwa_approvals (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  asset_id UUID NOT NULL REFERENCES public.rwa_assets(id) ON DELETE CASCADE,
  action public.approval_status NOT NULL, -- pending/approved/rejected
...
CREATE POLICY "approvals insert" ON public.rwa_approvals FOR INSERT TO authenticated
...
CREATE POLICY "calc history insert" ON public.rwa_calculations FOR INSERT TO authenticated WITH CHECK (true);

-- Auto-calc trigger: rwa_amount = exposure * risk_weight
...
END;
$$;
CREATE TRIGGER trg_rwa_auto_calc BEFORE INSERT OR UPDATE ON public.rwa_assets
  FOR EACH ROW EXECUTE FUNCTION public.rwa_auto_calc();

-- History snapshot on any change
...
END;
$$;
CREATE TRIGGER trg_rwa_history AFTER INSERT OR UPDATE ON public.rwa_assets
  FOR EACH ROW EXECUTE FUNCTION public.rwa_history_snapshot();

-- Seed baseline Basel-style risk weight rules
INSERT INTO public.risk_weight_rules (category, asset_class, counterparty_type, rating, risk_weight, description) VALUES
  ...
  ('operational', 'Basic Indicator', NULL, NULL, 0.15, 'Basic indicator approach (15% of gross income)'),
